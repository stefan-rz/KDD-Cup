...
"""
    召回模块
"""
def topk_recall_glove_embeeding(
                                click_all
                                ,dict_label
                                ,k=100
                                ,dim=88
...
def topk_recall_association_rules(
    ...
    hot_list = list(click_all['item_id'].value_counts().index[:1000].values)
    ...
    for i, row in tqdm(data_.iterrows()):
        ...
        if len(dict_item_id_score_topk) < k:
            for i, item in enumerate(hot_list):
                if (item not in list_item_id) and (item not in dict_item_id_score_topk):
                    item_similar = item
                    score_similar = - i - 100 
                    dict_item_id_score_topk.append( (item_similar, score_similar) )
                if len(dict_item_id_score_topk) == 50:
                    break
...
"""
    Glove 特征
"""
def matrix_glove_embeeding(click_all,flag,mode,threshold=0,dim=100,epochs=30,learning_rate=0.5):
    """
        glove 原理 + 矩阵分解：
            窗口内 加权统计 共线性词频
    ...
    if mode == 'only':
        glove_embeeding = {flag:{}}
    if mode == 'all':
        glove_embeeding = {'user':{},'item':{}}
    for k in keys:
        if 'all' not in k:
            glove_embeeding[flag][k] = glove.word_vectors[glove.dictionary[k]]
        if 'all' in k:
            flag_ = group_by_col.split('_')[0]
            k_ = k.split('_')[1]
            glove_embeeding[flag_][k_] = glove.word_vectors[glove.dictionary[k]]
            
    return glove_embeeding
...
def get_train_test_data(
                        topk_recall,
                        dict_glove_embeeding_all_ui_item,
                        dict_glove_embeeding_all_ui_user,
                        dict_glove_embeeding_item_only,
                        dict_glove_embeeding_user_only,
                        flag_test=False
                        ):
    ...
    for flag, data in list_train_test:
        ...
        for i,row in tqdm(data.iterrows()):
            ...
            dim1_user = dict_glove_embeeding_all_ui_item['user'][user_id]
            dim1_item = dict_glove_embeeding_all_ui_item['item'][item_id]
            similarity_d1 =  np.sum(dim1_user/np.sqrt(np.sum(dim1_user**2)) * dim1_item/np.sqrt(np.sum(dim1_item**2)))

            dim2_user = dict_glove_embeeding_all_ui_user['user'][user_id]
            dim2_item = dict_glove_embeeding_all_ui_user['item'][item_id]
            similarity_d2 =  np.sum(dim2_user/np.sqrt(np.sum(dim2_user**2)) * dim2_item/np.sqrt(np.sum(dim2_item**2)))

            dim3_item = dict_glove_embeeding_item_only['item'][item_id]

            dim4_user = dict_glove_embeeding_user_only['user'][user_id]
...
def train_model(feature_all, recall_rate, hot_list, valid=0.2, topk=50, num_boost_round=1500, early_stopping_rounds=100):
...
for phase in range(0,now_phase + 1):  
    ...
    print('-------- 召回 -------------')
    """
    # glove embeeding 召回 太慢了 放弃
    topk_recall = topk_recall_glove_embeeding(
    ...
    dict_glove_embeeding_all_ui_item = matrix_glove_embeeding(
                                                            click_all=train_test,
                                                            flag='item',
                                                            mode='all',
                                                            dim=dim,
    ...
    dict_glove_embeeding_all_ui_user = matrix_glove_embeeding(
                                                            click_all=train_test,
                                                            flag='user',
                                                            mode='all',
                                                            dim=dim,
    ...
    dict_glove_embeeding_item_only = matrix_glove_embeeding(
                                                            click_all=train_test,
                                                            flag='item',
                                                            mode='only',
                                                            dim=dim,
    ...
    dict_glove_embeeding_user_only = matrix_glove_embeeding(
                                                            click_all=train_test,
                                                            flag='user',
                                                            mode='only',
                                                            dim=dim,
    ...
    print('------- 训练模型 -----------')
    submit = train_model(feature_all, recall_rate=hit_rate, hot_list=hot_list, valid=0.2, topk=50, num_boost_round=1, early_stopping_rounds=1)
